...
export const assemble = ({ stack, fresh }: Env, tag: string, npop: number): Env => {
  const [tail, front] = cut(stack, npop);
  return push(
    { stack: tail, fresh },
    createElement(tag, front.map(item => item.elem))
  );
};
...
const pushTokenNode = (tag: string) =>
...
export const pushMi: (env: Env, ident: string) => Env
  = pushTokenNode('mi');

export const pushMo: (env: Env, operator: string) => Env
  = pushTokenNode('mo');

export const packInvisibleTimes = ({ stack, fresh }: Env, nchd: number): Env => {
  ...
  const children = front.reduce<JSX.Element[]>(
    (chd, item, i) => (
      i === 0
        ? [item.elem]
        : chd.concat([
          createElement('mo', '&InvisibleTimes;'),
          item.elem
        ])
    ),
    []
